...
此时打开你的文件夹（这里是`\MaiMBot\mainonebot-adapter`），手动把下载的的`\nonebot-plugin-maibot-adapters`下的`nonebot_plugin_maibot_adapters`文件夹复制到`\MaiMBot\mainonebot-adapter\src\plugins`下。
